...
namespace Common.Presentation.Validation
{
    public class PaginationValidator : AbstractValidator<PaginationValParams>
    {
        ...
        {
            RuleFor(x => x.Page)
                .GreaterThan(0).WithErrorCode(CommonPresentationErrors.Validation.Pagination.Page);

            RuleFor(x => x.PageSize)
                .GreaterThan(0).WithErrorCode(CommonPresentationErrors.Validation.Pagination.PageSize);
        }
    }
